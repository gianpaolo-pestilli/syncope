...
public class DefaultPasswordGenerator implements PasswordGenerator {
    ...
    protected static final Logger LOG = LoggerFactory.getLogger(DefaultPasswordGenerator.class);
    ...
    protected DefaultPasswordRuleConf merge(final List<DefaultPasswordRuleConf> defaultRuleConfs) {
        ...
        defaultRuleConfs.forEach(ruleConf -> {
            mergeLengths(result, ruleConf);
            mergeCharacterCounts(result, ruleConf);
            mergeLists(result, ruleConf);
            mergeFlags(result, ruleConf);
        });
    ...
    private void mergeLengths(final DefaultPasswordRuleConf result, final DefaultPasswordRuleConf ruleConf) {
    ...
    private void mergeCharacterCounts(final DefaultPasswordRuleConf result, final DefaultPasswordRuleConf ruleConf) {
    ...
    private void mergeLists(final DefaultPasswordRuleConf result, final DefaultPasswordRuleConf ruleConf) {
    ...
    private void mergeFlags(final DefaultPasswordRuleConf result, final DefaultPasswordRuleConf ruleConf) {
        if (!result.isUsernameAllowed()) {
            result.setUsernameAllowed(ruleConf.isUsernameAllowed());
        }
    }

    private void validate(final DefaultPasswordRuleConf ruleConf) {
        if (ruleConf == null) {
            throw new IllegalArgumentException("Configuration cannot be null");
        }
        if (ruleConf.getMinLength() < 0) {
            throw new IllegalArgumentException("Minimum length cannot be negative");
        }
        if (ruleConf.getMaxLength() < 0) {
            throw new IllegalArgumentException("Maximum length cannot be negative");
        }
        if (ruleConf.getMaxLength() > 0 && ruleConf.getMinLength() > ruleConf.getMaxLength()) {
            throw new IllegalArgumentException("Minimum length cannot be greater than maximum length");
        }
        if (ruleConf.getMaxLength() > 0) {
            int requiredLength = ruleConf.getAlphabetical() + ruleConf.getUppercase() +
                    ruleConf.getLowercase() + ruleConf.getDigit() + ruleConf.getSpecial();
            if (requiredLength > ruleConf.getMaxLength()) {
                throw new IllegalArgumentException("Strict cardinality requirements exceed maximum length");
            }
        }

        boolean hasConflict = ruleConf.getSpecialChars().stream()
                .anyMatch(c -> ruleConf.getIllegalChars().contains(c));
        if (hasConflict) {
            throw new IllegalArgumentException("Conflict between special and illegal characters");
        }
    }
