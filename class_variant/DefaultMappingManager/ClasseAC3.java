        ...
        try {
            ...
            if (ina.getField() != null && !values.isEmpty() && values.getFirst() != null) {
                ...
                if (anyTO instanceof GroupableRelatableTO g && ina.getMembership() != null) {
                    groupDAO.findByName(ina.getMembership()).ifPresent(g -> {
                        MembershipTO m = g.getMembership(g.getKey()).orElseGet(() -> {
                            MembershipTO nm = new MembershipTO.Builder(g.getKey()).build(); g.getMemberships().add(nm); return nm;
                        });
